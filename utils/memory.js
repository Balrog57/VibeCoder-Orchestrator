...
export async function initMemory(basePath) {
    ...
    const vectorPath = path.join(memoryPath, 'vector');
    
    try {
        await fs.mkdir(memoryPath, { recursive: true });
        await fs.mkdir(sessionsPath, { recursive: true });
        await fs.mkdir(weeklyPath, { recursive: true });
        await fs.mkdir(vectorPath, { recursive: true });
...
function getVectorConfig() {
    const backend = (process.env.MEMORY_VECTOR_BACKEND || 'json').toLowerCase();
    const model = (process.env.MEMORY_EMBED_MODEL || 'hash-v1').toLowerCase();
    const dim = Math.max(64, Math.min(1024, parseInt(process.env.MEMORY_VECTOR_DIM || '256', 10) || 256));
    return { backend, model, dim };
}

function embedTextLocal(text, config) {
    const normalized = (text || '').toLowerCase().replace(/\s+/g, ' ').trim();
    const dim = config.dim;
    const vector = Array.from({ length: dim }, () => 0);
    if (!normalized) return vector;

    // Local deterministic embedding: hashed char trigrams into a fixed-size vector.
    for (let i = 0; i < Math.max(1, normalized.length - 2); i++) {
        const trigram = normalized.slice(i, i + 3);
        const hash = createHash('sha1').update(`${config.model}:${trigram}`).digest();
        const idx = (hash[0] * 256 + hash[1]) % dim;
        vector[idx] += 1;
    }
...
function createTextSnippet(content, maxChars = 420) {
    const compact = content.replace(/\s+/g, ' ').trim();
    return compact.length <= maxChars ? compact : `${compact.slice(0, maxChars)}...`;
}

async function loadVectorIndex(memoryPath) {
    const indexPath = path.join(memoryPath, 'vector', 'index.json');
    try {
        const raw = await fs.readFile(indexPath, 'utf8');
        const parsed = JSON.parse(raw);
        return parsed;
    } catch {
        return { version: 1, backend: 'json', model: 'hash-v1', dim: 256, docs: [] };
    }
}

async function saveVectorIndex(memoryPath, indexData) {
    const indexPath = path.join(memoryPath, 'vector', 'index.json');
    await fs.mkdir(path.dirname(indexPath), { recursive: true });
    await fs.writeFile(indexPath, JSON.stringify(indexData), 'utf8');
}

async function refreshVectorIndex(memoryPath, config) {
    const allMdFiles = await collectMarkdownFiles(memoryPath, memoryPath);
    const candidates = allMdFiles.filter(f => !f.relativePath.startsWith('xdg-cache/') && !f.relativePath.startsWith('xdg-config/'));

    const docs = [];
    for (const fileInfo of candidates) {
        try {
            const stat = await fs.stat(fileInfo.absolutePath);
            const content = await fs.readFile(fileInfo.absolutePath, 'utf8');
            const snippet = createTextSnippet(content);
            docs.push({
                file: fileInfo.relativePath,
                mtimeMs: stat.mtimeMs,
                embedding: embedTextLocal(content, config),
                snippet
            });
        } catch {
    ...
    await saveVectorIndex(memoryPath, indexData);
    return indexData;
}

async function queryVectorMemory(memoryPath, prompt) {
    const config = getVectorConfig();
    const effectiveConfig = { ...config };
    if (config.backend !== 'json') {
        // Unsupported backends fallback to JSON for now while keeping config explicit.
        effectiveConfig.backend = 'json';
    }

    let index = await loadVectorIndex(memoryPath);
    if (!index.docs || index.model !== effectiveConfig.model || index.dim !== effectiveConfig.dim || index.docs.length === 0) {
        index = await refreshVectorIndex(memoryPath, effectiveConfig);
    }

    if (!index.docs.length) return '';

    const qvec = embedTextLocal(prompt, effectiveConfig);
    ...
    return ranked.map(r => `[${r.file}] (vector=${r.score.toFixed(3)}): ${r.snippet}`).join('\n\n');
}
...
export async function queryMemory(basePath, prompt) {
    ...
    // Fallback 1: recherche vectorielle locale
    try {
        const vectorContext = await queryVectorMemory(memoryPath, prompt);
        if (vectorContext && vectorContext.trim()) {
            console.log('[Memory] Vector fallback: résultats trouvés');
            return vectorContext;
        }
    } catch (err) {
        console.warn(`[Memory] Vector fallback échec: ${err.message}`);
    }

    // Fallback 2: recherche textuelle simple
    try {
...
function getWeeklyLogPath(memoryPath, date = new Date()) {
